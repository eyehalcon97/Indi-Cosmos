...
propiedad::propiedad(indigo_property *property,QWidget *parent){
    this->parent= parent;
    this->device= string(property->device);
    ...
    switch(type){
        case 1:
            ...
            for(int i=0;i<count;i++){
                itemstexto[i] = new itemtexto(&property->items[i],parent);
            }
            break;
        case 2:
            itemsnumero = new itemnumero*[count];
            for(int i=0;i<count;i++){
                itemsnumero[i] = new itemnumero(&property->items[i],parent);
            }
            break;
        case 3:
            itemsswitch = new itemswitch*[count];
            for(int i=0;i<count;i++){
                itemsswitch[i] = new itemswitch(&property->items[i],parent);
            }
            break;
        case 4:
            itemslight = new itemlight*[count];
            for(int i=0;i<count;i++){
                itemslight[i] = new itemlight(&property->items[i],parent);
            }
            break;
        case 5:
            itemsblob = new itemblob*[count];
            for(int i=0;i<count;i++){
                itemsblob[i] = new itemblob(&property->items[i],parent);
            }
            break;
    }
}
